...
function HospitalRequests(){
  ...
  useEffect(() => {
    ...
    useEffect(() => {
        ...
        axios
            .get(`http://88.200.63.148:30031/hospital/requests/${user.id}`,{
                withCredentials: true
            })
            .then((res) => {
    ...
    <main class="notifications-list">
        ...
        {completedRequests.map((c) => (
            ...
            <div class="details-grid">
                ...
                <div class="detail-item">
                    <label>Urgency</label>
                    <p class="urgency-text">{c.urgency_level}</p>
                </div>
                <div class="detail-item">
                    <label>Status</label>
                    <span class="status-done">{c.status}</span>
                </div>
                <div class="detail-item">
                    <label>Request Date</label>
                    <p class="date-text">{new Date(c.created_at).toLocaleDateString()}</p>
                </div>
            </div>
            <button className="edit-btn" onClick={() => navigate(`/hospital/requests/edit-request/${c.request_id}`)}>
